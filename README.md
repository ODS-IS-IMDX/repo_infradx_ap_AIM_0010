...
| 機能名                                   | 機能概要                                                                                                       |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| 認可情報登録 | 認可グループとそのグループが参照可能なレイヤの情報を認可レイヤ（設備）に登録する。既に参照可能なレイヤが登録されている場合は、既存データを削除した上で登録を行う。 |
...
3. 依存リポジトリの使用準備が完了していることが必要です。
    - 依存リポジトリ
        ...
        ※詳細は下記の[リポジトリ一覧](https://github.com/ODS-IS-IMDX#%E3%83%AA%E3%83%9D%E3%82%B8%E3%83%88%E3%83%AA%E4%B8%80%E8%A6%A7)を参照
...
6. 実行ログを確認します。
    ...
    ```
    [2026-02-03 18:54:58,621] [INFO] [AIM] BPI0001:AIM_0010_認可情報登録処理 開始
    [2026-02-03 18:54:58,710] [INFO] [AIM] BPI0002:AIM_0010_認可情報登録処理 終了
    ```
